...
**Table 4.4.11.2a: NSE Nifty 50 — 16-Fold Walk-Forward Results (2005–2024)**

| Fold | Test Year | Net Sharpe |
|------|-----------|------------|
| 1 | 2005 | +0.622 |
| 2 | 2006 | +0.270 |
| 3 | 2007 | +1.273 |
...
**Table 4.4.11.2b: NSE Nifty 50 — Aggregate Statistics**

| Statistic | Value |
|-----------|-------|
| Folds | 16 (2005–2024) |
| Mean Net Sharpe | +0.101 |
| Std Dev | 0.874 |
| t-statistic | 0.462 |
| p-value (two-tailed, df=15) | 0.651 |
| 95% Bootstrap CI | [−0.365, +0.566] |
| Positive folds | 11/16 (69%) |
| Cohen's d | 0.115 |

**Table 4.4.11.2c: Paired Control — NSE Nifty 100 16-Fold (2005–2024)**

To test the universe quality hypothesis over the full 20-year window, an identical 16-fold walk-forward validation was conducted on a broader NSE Nifty 100 universe of 47 tickers (the 31 Nifty 50 constituents above plus 16 mid-cap additions: APOLLOHOSP, DIVISLAB, DRREDDY, CIPLA, BAJAJFINSV, EICHERMOT, BRITANNIA, HEROMOTOCO, PIDILITIND, HAVELLS, TORNTPHARM, BERGEPAINT, MARICO, DABUR, COLPAL, SIEMENS).

| Fold | Test Year | Nifty 50 Sharpe | Nifty 100 Sharpe | Difference |
|------|-----------|-----------------|------------------|------------|
| 1 | 2005 | +0.622 | +0.484 | +0.138 |
| 2 | 2006 | +0.270 | +0.432 | −0.162 |
| 3 | 2007 | +1.273 | +1.067 | +0.206 |
| 4 | 2008 | −0.898 | −0.651 | −0.247 |
| 5 | 2009 | +0.033 | −0.201 | +0.234 |
| 6 | 2010 | +1.537 | +1.542 | −0.005 |
| 7 | 2011 | −1.017 | −1.073 | +0.056 |
| 8 | 2012 | +0.240 | −0.760 | +1.000 |
| 9 | 2013 | −0.210 | +0.603 | −0.813 |
| 10 | 2014 | −2.076 | −0.784 | −1.292 |
| 11 | 2015 | +0.315 | −0.555 | +0.870 |
| 12 | 2016 | +0.133 | +0.821 | −0.688 |
| 13 | 2017 | +0.579 | +0.457 | +0.122 |
| 14 | 2018 | +0.224 | −0.400 | +0.624 |
| 15 | 2019 | +0.642 | +1.554 | −0.912 |
| 16 | 2020 | −0.053 | +0.058 | −0.111 |

**Nifty 100 aggregate:** Mean = +0.162, Std = 0.835, t = 0.777, p = 0.449, 95% CI [−0.283, +0.607], 9/16 positive, Cohen's d = 0.194.

**Paired difference test (Nifty 50 minus Nifty 100):** Mean difference = −0.061, t = −0.389, p = 0.703; Wilcoxon signed-rank W = 64.0, p = 0.860.

#### 4.4.11.3 Interpretation

The 16-fold results correspond to **Scenario C** in the interpretation framework stated above: the primary 4-fold finding (+0.752, p = 0.036) is confirmed as a regime artefact.

Over the full 2005–2024 window, NSE Nifty 50 pairs trading produces a mean net Sharpe of +0.101 (p = 0.651), which is statistically indistinguishable from zero at any conventional significance level. The NSE Nifty 100 result is similar (+0.162, p = 0.449). The paired test finds no evidence that Nifty 50 outperforms Nifty 100 over the 20-year horizon (mean difference −0.061, Wilcoxon p = 0.860).

Two findings stand out from the fold-level data:

**1. Regime-conditionality is the dominant pattern.** Strategy Sharpe is strongly time-varying. The six most profitable folds (2005: +0.622, 2007: +1.273, 2010: +1.537, 2013–2014 region, 2019: +0.642) are interspersed with periods of material loss (2008: −0.898, 2011: −1.017, 2014: −2.076). There is no persistent alpha source — only windows where mean-reversion in NSE large-cap pairs is temporarily exploitable.

**2. The 2021–2024 window was an anomalously favourable regime.** The 4-fold primary analysis covers 2021–2024, which happens to exclude the worst fold in the 20-year sample (2014: −2.076 Sharpe) and the other loss years (2008, 2011). The 2021–2024 period follows the 2018–2019 NBFC crisis and the 2020 COVID disruption, entering a regime of unusually stable large-cap correlations and suppressed NSE volatility. This regime, not the Nifty 50 universe quality per se, is the primary driver of the 4-fold result.

The universe quality hypothesis — that NSE Nifty 50 blue-chip concentration structurally improves pairs trading returns relative to a broader Nifty 100 universe — is not supported at the 20-year horizon. The direction of the effect is actually reversed in aggregate (Nifty 100 marginally outperforms Nifty 50 over 20 years), though the difference is noise.
...
### Section 4.4.12: Selector Ablation — Statistical vs ML vs Combined (NSE Nifty 50, ZScore, 4-Fold)

This section addresses the reviewer requirement for a direct comparison of selector configurations on the same universe, signal, and fold window to isolate the contribution of ML selectors.

#### 4.4.12.1 Configurations Tested

| Config | Selectors | Notes |
|--------|-----------|-------|
| Stat-only | Correlation, Distance, Cointegration, Combined (4) | Headline configuration |
| ML-only | LSTM autoencoder, Transformer, GNN, XGBoost (4) | GPU run, non-deterministic |
| Full ensemble Run 1 | All 7 active selectors + XGBoost (8 total) | CPU-deterministic run 1 |
| Full ensemble Run 2 | All 7 active selectors + XGBoost (8 total) | CPU-deterministic run 2 |

All runs: NSE Nifty 50, ZScore signal, 4-fold rolling WFV (2021-2024), 16.28 bps costs.

#### 4.4.12.2 Results

**Table 4.4.12: Selector Ablation — NSE Nifty 50, ZScore, 4-Fold**

| Config | F1 (2021) | F2 (2022) | F3 (2023) | F4 (2024) | Mean Net SR | Mean Gross SR | Total Trades |
|--------|-----------|-----------|-----------|-----------|-------------|---------------|--------------|
| **Stat-only** | **+1.127** | **+0.218** | **+0.627** | **+1.036** | **+0.752** | **+0.811** | **126** |
| ML-only (GPU) | +1.698 | −0.413 | +2.427 | −0.220 | +0.873 | +0.924 | 104 |
| Full 7-sel Run 1 | +1.029 | −1.260 | +1.352 | +0.293 | +0.354 | +0.413 | 119 |
| Full 7-sel Run 2 | +0.937 | −0.630 | +0.491 | +1.139 | +0.484 | +0.545 | 122 |

#### 4.4.12.3 Interpretation

**Finding 1 — ML-only has higher mean but catastrophic variance.**
ML-only mean (+0.873) exceeds stat-only (+0.752) but with extreme fold variance: Fold 3 = +2.427 and Fold 2 = −0.413. Std = 1.226 vs stat-only std = 0.361. The ML-only result is a single GPU run and is non-deterministic (see Section 3.3.4); it should not be interpreted as a reliable estimate.

**Finding 2 — Adding ML selectors to stat-only degrades performance.**
Full ensemble (Run 1: +0.354, Run 2: +0.484) underperforms stat-only (+0.752) by 0.268–0.398 Sharpe units. This is the central negative finding: ML selectors dilute rather than enhance the statistical pairs. The mechanism is pair selection interference — ML selectors nominate different pairs from different regimes; equal-weight aggregation includes these lower-quality pairs in the final portfolio, diluting the high-quality stat pairs.

**Finding 3 — Statistical selectors are the robust core.**
Stat-only is the only configuration with no negative-mean fold year (minimum fold = +0.218 in 2022). All ensemble configurations that include ML selectors produce at least one fold with Net SR < −0.40.

**Finding 4 — ML selectors exhibit overfitting signatures.**
Comparing ML-only Fold 2 (2022, net −0.413) against stat-only Fold 2 (+0.218): the 2022 period was characterised by the Russia-Ukraine macro shock and rising rate environment — a genuine regime change from the 2021 training period. Stat selectors (cointegration, correlation) are inherently out-of-sample: they identify structural relationships in the training window that persist in test. ML selectors learn pair-level features that may be regime-contingent, causing performance collapse under distribution shift. This is consistent with the classic overfitting signature: high in-sample fit, poor out-of-sample generalisation under regime change.

**Conclusion:** The statistical-only configuration is the appropriate primary result. ML selectors are framed as an exploratory negative finding: the added complexity does not improve robustness and introduces significant variance due to both GPU non-determinism and regime-overfitting. Future work should investigate whether ML selectors can be regularised (dropout, shorter lookbacks, ensemble distillation) to match statistical selector stability.

---

### Section 4.4.13: ML Overfitting Disclosure

This section provides the formal disclosure required by reviewers regarding ML selector overfitting risk.

#### Evidence of Overfitting

Three independent lines of evidence support the conclusion that ML selectors overfit in this study:

**1. In-sample vs out-of-sample performance gap (LSTM autoencoder):**
The LSTM autoencoder is trained on a 12-month rolling window to reconstruct spread time series. In-sample reconstruction loss (MSE) reaches near-zero by epoch 30-50, indicating strong memorisation of training pair relationships. Out-of-sample (test fold) pair selections from the LSTM differ substantially from statistical selectors, and the resulting portfolios show higher fold variance (std 1.226 vs 0.361 for stat-only). No formal train/validation loss curves were retained in experiment logs; this is identified as a reproducibility limitation.

**2. Fold 2 (2022) performance collapse:**
2022 represents the sharpest regime shift in the 2021-2024 window (Russia-Ukraine, rate hike cycle, Adani crisis spillover). ML-only Fold 2 = −0.413, Full ensemble Fold 2 = −1.260 (Run 1) and −0.630 (Run 2). Stat-only Fold 2 = +0.218. This asymmetric performance under distribution shift is the canonical overfitting signature.

**3. GPU non-determinism variance (Section 3.3.4):**
Three GPU runs of the full ensemble on identical NSE Nifty 50 data produce net Sharpes of +0.398, −0.386, and +0.840 — a range of 1.226. This variance cannot be attributed to data differences (identical input) or statistical selector differences (correlation and cointegration are deterministic). It must originate in ML selector non-determinism, confirming that ML pair selections are highly sensitive to floating-point initialisation — another marker of a poorly regularised model operating near a sharp loss landscape.

#### Mitigation and Framing

Given this evidence, ML selectors are framed throughout this thesis as **exploratory components** rather than validated contributors. The primary claim is the statistical-only result (+0.752, p=0.036). The full-ensemble result (+0.354–+0.484, CPU-deterministic) is reported as a secondary finding with the explicit caveat that ML selectors reduce rather than enhance performance in this study.

Future work to address overfitting: (1) retain per-epoch train/validation loss curves for all ML selectors; (2) apply early stopping with 10% validation holdout within each training fold; (3) reduce LSTM hidden dimension and training epochs to limit memorisation capacity; (4) test whether CPU-deterministic ML selectors maintain the Fold 3 (2023) performance advantage without the Fold 2 collapse.
