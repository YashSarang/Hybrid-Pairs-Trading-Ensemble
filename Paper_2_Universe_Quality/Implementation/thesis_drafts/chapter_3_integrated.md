...
**Academic Benchmarks:**
- Gatev et al. (2006): +0.80 Sharpe (US, 1963-1997, no transaction costs)
...
**Chapter 4's multi-market validation will demonstrate that universe quality (Nifty 50) is the primary performance driver**: India+ZScore honest 3-run mean = +0.284 Sharpe (CPU range +0.353-+0.484; best GPU run +0.840 = cherry-picked, exploratory). Honest multiplier = 5.5x (mean +0.284 / +0.052). Under period-matched arithmetic, methodology improvement (+0.461) marginally exceeds geographic premium (+0.368). The 16x multiplier and 'geographic alpha dominates' framing are retracted.
