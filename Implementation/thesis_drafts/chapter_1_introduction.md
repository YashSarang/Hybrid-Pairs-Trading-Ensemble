...
**Hypothesis:** An ensemble of 8 selectors—combining statistical methods (correlation, Euclidean distance, cointegration, copula) with deep learning architectures (LSTM autoencoders, Transformers, Graph Neural Networks, Variational Autoencoders)—will produce more robust pair selections that remain profitable under Indian cost structures.
...
### RQ3: Does Geographic Diversification Dominate Methodology Optimization?
...
**Expected Finding:** Geographic alpha (market selection effect) > methodological alpha (algorithm tuning effect).
...
### Primary Contribution: Geographic Alpha Dominates Methodology Optimization

**Finding:** Multi-market India (Nifty 50 + ZScore) achieves **+0.840 Sharpe ratio**, 16.2x better than optimized rolling NSE (+0.052) and 305% better than baseline expanding NSE (-0.409).
...
- Same transaction costs (16.4 bps) but 16x better risk-adjusted returns
...
**Novel Insight:** Geographic diversification improvement (+0.788 Sharpe, from +0.052 NSE rolling to +0.840 India) is **1.7x larger** than methodology improvement (+0.461 Sharpe, from -0.409 NSE expanding to +0.052 NSE rolling). This quantifies the relative importance of market vs method for the first time in pairs trading literature.
...
**Finding:** The 8-selector ensemble (4 statistical + 4 ML) successfully generates trades in all 7 multi-market experiments (US/India/Brazil/UK × ZScore/OU), demonstrating framework portability.

**Technical Achievement:**
- **Statistical selectors** (Correlation, Euclidean Distance, Cointegration, Copula) work universally across markets without retraining
- **ML selectors** (LSTM autoencoder, Transformer, GNN, VAE) adapt to different volatility regimes and liquidity profiles
...
- **Section 3.1-3.2:** Data description (NSE Nifty 100, 2014-2025, daily adjusted prices)
- **Section 3.3-3.5:** Selector descriptions (8 statistical + ML models), ensemble aggregation, signal generation (ZScore/OU), entry/exit logic, risk management
- **Section 3.6:** Rolling vs Expanding window validation (6-fold walk-forward, statistical tests)
...
2. **Overfitting Risk:** 8 selectors + 2 signal models + multiple markets → high degrees of freedom (no out-of-sample test beyond 2025)
...
- **Time Period:** 2014-2025 includes COVID crash, QE era, inflation shock—results may not hold in different regimes
...
**For Academics:**
- Establishes that geographic alpha > methodological alpha (16.2x multiplier quantifies the effect)
...
**For Practitioners:**
- Identifies India Nifty 50 as a high-alpha pairs trading market (+0.840 Sharpe after costs)
...
This work contributes to the **market efficiency debate**: if pairs trading remains profitable in India (+0.840 Sharpe) but fails in the US (-0.297 Sharpe) using identical methodology, it suggests that:
