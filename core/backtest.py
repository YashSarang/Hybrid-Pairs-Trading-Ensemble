...
def _metrics_from_pnl(
    pnl: pd.Series, capital: float, periods_per_year: int
) -> Dict[str, float]:
    if pnl.empty:
...
class IndianCosts:
    ...
    def round_trip_cost_fraction(self) -> float:
        ...
        slippage_bps = 2 * self.slippage_bps_per_leg
...
class BacktestResult:
    ...
    equity_net: pd.Series       # capital + cumsum(net_pnl)
    ...
    metrics: Dict[str, float]   # Gross.* and Net.* keys
    params: Dict                # notional_each & costs
...
def backtest_pairs(
    ...
    notional_each = float(
        min(cfg.capital / max(1, cfg.max_concurrent_pairs), cfg.per_trade_cap)
    )
    ...
    for p in selected_pairs:
        ...
        turnover_series = turnover_series.add(
            pair_turnover.reindex(index).fillna(0.0), fill_value=0.0
        )
                ...
                pd.DataFrame(
                    {
                        ...
                        "signal": sig_scaled.loc[trans_idx].values,
                    }
                ).set_index("time")
            )
    ...
    trades = (
        pd.concat(all_trades) if all_trades else pd.DataFrame(columns=["pair", "signal"])
    )
    params = {
