...
**Key Result:** Trading pairs in India's Nifty 50 index produced a Sharpe ratio of +0.840 (meaning you earn 84 cents of profit per unit of risk), which is **16 times better** than the same strategy on India's broader Nifty 100 index. Meanwhile, the exact same system **lost money** in the United States (-0.297 Sharpe).
...
This thesis demonstrates that geographic diversification dominates methodology optimization in pairs trading. Using a hybrid ensemble of 8 selectors (statistical + machine learning) validated across 4 markets (India, US, Brazil, UK) with 6-fold walk-forward testing (2014-2025), we find that multi-market India (Nifty 50) achieves net Sharpe +0.840—16.2x better than optimized rolling NSE (+0.052) and 305% better than baseline expanding NSE (-0.409). Geographic alpha (+0.788) is 1.7x larger than methodology alpha (+0.461), proving that **WHERE we trade matters more than HOW we trade**. Rolling-window training improves NSE by 113%, but the gain is entirely cost-driven (73% fewer trades) and statistically non-significant (p = 0.320). The ensemble generalizes across all markets, but profitability follows efficiency ordering: India >> Brazil > UK > US (US fails at -0.297 net). For practitioners, we establish that gross Sharpe > +0.90 is required for net > +0.80 under 16.4 bps Indian transaction costs. For academics, we provide first multi-market evidence quantifying geographic vs methodological contributions, challenge algorithm-centric research priorities, and document ML non-determinism (TensorFlow GPU randomness) as a reproducibility concern.
